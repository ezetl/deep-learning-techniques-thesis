 ...
 *
 * This code generates labels regarding the similarity of the images. For
 * SFA based pretraining, image pairs with relative translation
 * in the range [-1, 1] and relative rotation within [-3º, 3º]
 * were considered to be temporally close to each other
 *
...
vector<DataBlob> process_images(vector<Mat> &list_imgs, unsigned int amount_pairs)
{
    ...
    for (unsigned int i=0; i<list_imgs.size(); i++)
    {
        ...
        for (unsigned int j=0; j<amount_pairs; j++)
        {
            ...
            d.z = rand_index;
            // Calculate the real index of the array of rotations (0 to 60)
            rand_index *= 3;
