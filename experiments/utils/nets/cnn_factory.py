...
class KITTINetFactory:
    ...
    def siamese_contrastive(lmdb_path=None, labels_lmdb_path=None,
        ...
        data, n.label = input_layers(lmdb_path=lmdb_path, labels_lmdb_path=labels_lmdb_path, batch_size=batch_size, scale=scale, is_train=is_train)

        # Slice data/labels
        n.data0, n.data1 = L.Slice(data, slice_param=dict(axis=1, slice_point=3), ntop=2)

        # BCNN
        pool5, pool5_p = bcnn(n.data0, n.data1, n, learn_all, False)

        # TCNNs
        n.fc1 = L.InnerProduct(pool5, num_output=1000, param=[weight_param('fc1_p_w', learn_all=True), bias_param('fc1_p_b', learn_all=True)], weight_filler=weight_filler_fc, bias_filler=bias_filler)
        n.relu6 = L.ReLU(n.fc1, in_place=True)
        n.dropout1 = L.Dropout(n.relu6, in_place=True)
        n.fc2 = L.InnerProduct(n.relu6, num_output=500, param=[weight_param('fc2_p_w', learn_all=True), bias_param('fc2_p_b', learn_all=True)], weight_filler=weight_filler_fc, bias_filler=bias_filler)

        n.fc1_p = L.InnerProduct(pool5_p, num_output=1000, param=[weight_param('fc1_p_w', learn_all=True), bias_param('fc1_p_b', learn_all=True)], weight_filler=weight_filler_fc, bias_filler=bias_filler)
        n.relu6_p = L.ReLU(n.fc1_p, in_place=True)
        n.dropout1_p = L.Dropout(n.relu6_p, in_place=True)
        n.fc2_p = L.InnerProduct(n.relu6_p, num_output=500, param=[weight_param('fc2_p_w', learn_all=True), bias_param('fc2_p_b', learn_all=True)], weight_filler=weight_filler_fc, bias_filler=bias_filler)
    ...
    def standar(lmdb_path=None, labels_lmdb_path=None, batch_size=125,
        ...
        if layers == '1':
            n.fc_intermediate = L.InnerProduct(n.norm1, num_output=num_classes, param=[weight_param('fc_intermediate_w', learn_all=True), bias_param('fc_intermediate_b', learn_all=True)], weight_filler=weight_filler_fc, bias_filler=bias_filler)
            if is_train:
        ...
        if layers == '2':
            n.fc_intermediate = L.InnerProduct(n.norm2, num_output=num_classes, param=[weight_param('fc_intermediate_w', learn_all=True), bias_param('fc_intermediate_b', learn_all=True)], weight_filler=weight_filler_fc, bias_filler=bias_filler)
            if is_train:
                n.loss = L.SoftmaxWithLoss(n.fc_intermediate, n.label)
            n.acc = L.Accuracy(n.fc_intermediate, n.label, include=dict(phase=caffe.TEST))
            return n, ('loss',), ('acc',)
        ...
        if layers == '3':
            n.fc_prev = L.InnerProduct(n.relu3, num_output=1000, param=[weight_param('fc_prev_w', learn_all=True), bias_param('fc_prev_b', learn_all=True)], weight_filler=weight_filler_fc, bias_filler=bias_filler)
            n.relu_prev = L.ReLU(n.fc_prev, in_place=True)
            n.fc_intermediate = L.InnerProduct(n.relu_prev, num_output=num_classes, param=[weight_param('fc_intermediate_w', learn_all=True), bias_param('fc_intermediate_b', learn_all=True)], weight_filler=weight_filler_fc, bias_filler=bias_filler)
            if is_train:
                n.loss = L.SoftmaxWithLoss(n.fc_intermediate, n.label)
            n.acc = L.Accuracy(n.fc_intermediate, n.label, include=dict(phase=caffe.TEST))
            return n, ('loss',), ('acc',)
        ...
        if layers == '4':
            n.fc_prev = L.InnerProduct(n.relu4, num_output=1000, param=[weight_param('fc_prev_w', learn_all=True), bias_param('fc_prev_b', learn_all=True)], weight_filler=weight_filler_fc, bias_filler=bias_filler)
            n.relu_prev = L.ReLU(n.fc_prev, in_place=True)
            n.fc_intermediate = L.InnerProduct(n.relu_prev, num_output=num_classes, param=[weight_param('fc_intermediate_w', learn_all=True), bias_param('fc_intermediate_b', learn_all=True)], weight_filler=weight_filler_fc, bias_filler=bias_filler)
            if is_train:
                n.loss = L.SoftmaxWithLoss(n.fc_intermediate, n.label)
        ...
        if not is_imagenet:
            if layers == '5':
                n.fc_prev = L.InnerProduct(n.pool5, num_output=1000, param=[weight_param('fc_prev_w', learn_all=True), bias_param('fc_prev_b', learn_all=True)], weight_filler=weight_filler_fc, bias_filler=bias_filler)
                n.relu_prev = L.ReLU(n.fc_prev, in_place=True)
                n.fc_intermediate = L.InnerProduct(n.relu_prev, num_output=num_classes, param=[weight_param('fc_intermediate_w', learn_all=True), bias_param('fc_intermediate_b', learn_all=True)], weight_filler=weight_filler_fc, bias_filler=bias_filler)
                if is_train:
                    n.loss = L.SoftmaxWithLoss(n.fc_intermediate, n.label)
            ...
            n.fc8 = L.InnerProduct(fc8input, num_output=num_classes, param=[weight_param('fc8_w', learn_all=True), bias_param('fc8_b', learn_all=True)], weight_filler=weight_filler_fc, bias_filler=bias_filler)
            ...
            n.fc8_imgnet = L.InnerProduct(fc8input_imgnet, num_output=num_classes, param=[weight_param('fc8_w', learn_all=True), bias_param('fc8_b', learn_all=True)], weight_filler=weight_filler_fc, bias_filler=bias_filler)
