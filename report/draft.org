...
\(D(G_W(x_1),G_W(x_2))\), lo que nos queda es una \textit{arquitectura
siamesa}. En nuestro dominio particular, \(G_W\) son redes
...
respecto a las entradas (Figura \ref{fig:siamese-diagram}). Las redes
siamesas no difieren en más que eso de las redes convolucionales que
ya vimos.
...
Luego, el problema puede ser implementado con una arquitectura
