...
netamente sobre los datos \textit{tal como están} (lo que no significa que
     los datos no necesiten preprocesamiento).
...
define lo que se llama una \textit{función de pérdida o costo}, mientras que
...
ellas con una técnica llamada \textit{descenso de gradiente estocástico}, 
que se basa en calcular el gradiente de un subconjunto
del total de datos (llamado \textit{mini-batch}) y actualizar \weights{} al final
de cada \textit{mini-batch}. Esto es muy útil dado que es
