...
con \(n\) igual número de canales que pueda tener una imagen
(usualmente \(1\) ó \(3\)). Es decir, toma coordenadas en
\(\mathbb{R}^2\) y devuelve la intensidad en ese punto.
...
\begin{tikzpicture}[every node/.style={minimum size=.5cm-\pgflinewidth, outer sep=0pt}]
    ...
    % Filter
    \draw[step=0.5cm,color=black] (6,0.5) grid (2,2);
    \node[opacity=1,black] at (3.75,1.75) {\tiny 50};
\end{tikzpicture}
\caption{}\label{fig:convmatrix}
\end{figure}
#+END_LaTeX
