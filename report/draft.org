...
Agrawal et al. \cite{LSM2015} ya han demostrado que es posible obtener
...
grandes suelen ser escasos, por ejemplo ImageNet \cite{imagenet}, COCO
\cite{DBLP:journals/corr/LinMBHPRDZ14} o SUN397 \cite{sun397}.
...
En esta sección se presentarán conceptos introductorios del
aprendizaje automático sobre los que definiremos diferentes algoritmos
comunes en tareas de entrenamiento, todos ellos enfocados en redes
neuronales. Se terminará la sección con una introducción a redes
...
Similar a la aterior, sólo que se le adiciona \(\lambda
\mathbf{||W||}_{1}\) a la función objetivo. Los pesos tienden a
converger a cero bajo la regularización L1. En general se prefiere la
regularización L2 por obtenerse mejores resultados.
...
calculamos el gradiente en cada variable utilizando la \textit{reglade la cadena}.
En la Figura \ref{fig:backprop} se puede observar los valores
computados para este ejemplo durante el \textit{forward pass} en verde
...
por clase se suele resolver el problema de la clasificación mediante
...
Se debe buscar entonces una función que relacione el espacio de
entrada con el de salida de tal manera que los elementos que son
semánticamente similares en el espacio de entrada estén cercanos en el
espacio de salida de acuerdo a alguna métrica de distancia (distancia
euclidea por ejemplo). Sea entonces \(G_W(x)\) una familia de
funciones con parámetros \(W\) donde \(x\) pertenece al espacio de
entrada, entonces buscamos encontrar \(W\) tal que la medida de
similitud \(D(G_W(x_1), G_W(x_2))\) sea pequeña si \(x_1\) y \(x_2\)
pertenecen a la misma categoría y grande en caso contrario. El sistema
se entrena con pares de elementos creados a partir del conjunto de
entrenamiento, buscando minimizar una función de costo que minimice
\(D(G_W(x_1), G_W(x_2))\) para pares cercanos semánticamente y a su
vez maximice esa distancia para pares lejanos.
...
Lo que acabamos de definir es una \textit{arquitectura siamesa}. En
nuestro dominio particular usaremos redes convolucionales como
nuestras \(G_W\) para definir la arquitectura siamesa por ser capaces
de generar buenas representaciones de imágenes a pesar de la gran
variación en color y geometría de las mismas.

Una red siamesa entonces consiste de dos redes \textit{gemelas} cuyos
parámetros \(W\) son compartidos. La arquitectura toma un par de
imágenes \((x_1, x_2)\) y computa si ambas pertenecen o no a la misma
categoría o si son similares de acuerdo a algún criterio. Para ello,
las representaciones obtenidas de ambas redes son redireccionadas a
una función de costo que calcula alguna métrica respecto a las
...
de clasificación \cite{siamese-oneshot} y recomendación de productos
\cite{ProductNet}.
...
*** Funciones de costo en redes siamesas
...
Queremos minimizar \(D(G_W(x_1), G_W(x_2))\) para los elementos que
sean semánticamente similares en el espacio de entrada (\textit{pares genuinos}). 
Podríamos pensar que definiendo una función de costo que minimice esa
distancia para los pares similares sería suficiente, sin embargo basta
con definir \(G_W(x_1)\) como una función constante para que la
función de costo sea cero. Por consiguiente, es necesario que la
función de costo tenga un término de \textit{contraste} que asegure
que no sólo se minimicen las distancias de los pares similares sino
que se aumenten las de los pares distantes semánticamente
(\textit{pares impostores})
...
pares genuinos. Definimos entonces una \textit{función de costo de
contraste} (\textit{contrastive loss} en inglés) para un par \(x_1,
...
representaciones que se obtienen de entrenar la red siamesa son
competitivas con las del estado del arte. La información disponible
para la red relacionada a la posición relativa de una foto y sus
transformaciones (traslación, etc) respecto al \textit{tiempo} se
denomina \textit{información odométrica}.

TODO: mas info sobre odometria
...
*** Midiendo similaridad entre imágenes: \textit{Slow Feature Analysis} y Automovimiento

Agrawal et al. \cite{LSM2015} proponen analizar el automovimiento del
agente para aprender \textit{features} útiles y lo comparan con otro
método muy utilizado en redes siamesas, \textit{Slow Feature Analysis}
(SFA). SFA es una técnica de aprendizaje no supervisado que se basa en
que las características relevantes cambian poco en una ventana de
tiempo pequeña. Aplicado a nuestro dominio donde un agente se mueve a
través de su entorno, podemos tomar representaciones de imágenes
\(x_{t_1}\) y \(x_{t_2}\) tomadas en tiempos \(t_1\) y \(t_2\)
respectivamente y computar su similitud mediante alguna métrica
\(D\). Si las imágenes están cercanas en el tiempo - es decir \(|t_1 -
t_2| \leq T\) para algún intervalo de tiempo \(T\) - son consideradas
similares y su distancia debería ser pequeña, caso contrario son
consideradas diferentes y su distancia debería ser mayor a algun
margen preestablecido. Podemos entonces establecer una función de
costo con el objetivo de penalizar las representaciones que son
...
mismo espacio. Podemos definir entonces a la función de costo SFA
como:
...
\cite{Geiger2012CVPR}, SUN397 \cite{sun397} e ILSVRC 2012
\cite{imagenet}.
...
pares de imágenes siguiendo los lineamientos de \cite{LSM2015}. Esto
significa que cada par está compuesto de la imagen original y la
...
tasa de aprendizaje de 0.01. Siguiendo los lineamientos de
\cite{LSM2015} se utilizaron márgenes \(m\) de 10 y 100 para SFA por
ser los que mejores resultados lograron. En ambas redes la tasa de
aprendizaje se reduce a la mitad cada 10000 iteraciones. El tamaño del
\textit{mini batch} fue de 125, lo cual equivale a procesar 5 millones
de pares de imágenes durante las 40000 iteraciones del entrenamiento.
...
al. \cite{LSM2015} a la hora de computar las transformaciones en la
...
*** TODO Agregar grafico SFA.
*** TODO Desarrollar automovimiento mas. Agregar ecuaciones de automovimiento (matriz de traslacion, etc).
*** TODO Agregar grafico automovimiento (la red con clasificadores on top)
...
*** TODO Agregar grafico de alexnet con la notacion utilizada
*** TODO Agregar las introducciones a cada capitulo.
** Datasets
