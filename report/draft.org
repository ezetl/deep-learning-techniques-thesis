...
compartidos. Una red siamesa toma un par de imágenes (\(x_1\) y
\(x_2\)) y computa si ambas pertenecen o no a la misma categoría o si
son similares. Para ello, las representaciones obtenidas de ambas
redes que componen la arquitectura siamesa son redireccionadas a una
función de costo que calcula alguna métrica respecto a las entradas
(Figura \ref{fig:siamese-diagram}). Las redes siamesas no difieren en
más que eso de las redes convolucionales que ya vimos.
...
*** TODO Desarrollar SFA mas. Agregar grafico
*** TODO Desarrollar automovimiento mas. Agregar grafico, Agregar ecuaciones de automovimiento (matriz de traslacion, etc). agregar algunas imagenes de ejemplo con las correspondientes traslaciones.
*** TODO Desarrollar mas redes siamesas. Basarse en papers. Desarrollar tambien mas los ejemplos
...
**** Función de costo de contraste

Queremos minimizar \(D(G_W(X_1), G_W(X_2))\) para los elementos que
sean semánticamente similares en el espacio de entrada (\textit{pares
genuinos}). Podríamos pensar que definiendo una función de costo que
minimice esa distancia para los pares similares sería suficiente, sin
embargo basta con definir \(G_W(X_1)\) como una función constante para
que la función de costo sea cero. Por consiguiente, es necesario que
la función de costo tenga un término de \textit{contraste} que asegure
...
pares genuinos. Definimos entonces una función de costo de contraste
para un par \(X_1, X_2\) de la siguiente manera:

\begin{equation}
L(W, (Y, X_1, X_2)^i) = (1 - Y) L_{gen}(G_W(X_1), G_W(X_2)) + Y L_{imp}(G_W(X_1), G_W(X_2))
\end{equation}
