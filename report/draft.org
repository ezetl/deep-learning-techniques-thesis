...
donde \(x\) pertenece al espacio de entrada, entonces buscamos
encontrar \(W\) tal que para una medida de similitud \(D\),
\(D(G_W(x_1), G_W(x_2))\) sea pequeña si \(x_1\) y \(x_2\) pertenecen
a la misma categoría y grande en caso contrario.

El sistema se entrena con pares de elementos creados a partir del
conjunto de entrenamiento, buscando optimizar una función de costo que
minimice \(D(G_W(x_1), G_W(x_2))\) para pares cercanos semánticamente
y a su vez maximice esa distancia para pares lejanos.
...
nuestro dominio particular, \(G_W\) son redes convolucionales. Su
elección radica en que son capaces de generar buenas representaciones
de imágenes a pesar de la gran variación en color y geometría de las
mismas.

Una red neuronal convolucional siamesa entonces consiste de dos redes
\textit{gemelas} cuyos parámetros \(W\) son compartidos. La
arquitectura toma un par de imágenes \((x_1, x_2)\) y computa si ambas
pertenecen o no a la misma categoría o si son similares de acuerdo a
algún criterio. Para ello, las representaciones obtenidas de ambas
redes son redireccionadas a una función de costo que calcula alguna
métrica respecto a las entradas (Figura
\ref{fig:siamese-diagram}). Las redes siamesas no difieren en más que
eso de las redes convolucionales que ya vimos.
...
(\textit{pares genuinos}). Podríamos pensar que definiendo una
función de costo que minimice esa distancia para los pares similares
sería suficiente, sin embargo basta con definir \(G_W(x_1)\) como una
función constante para que la función de costo sea cero. Por
consiguiente, es necesario que la función de costo tenga un término de
\textit{contraste} que asegure que no sólo se minimicen las distancias
de los pares similares sino que se aumenten las de los pares distantes
semánticamente (\textit{pares impostores})
...
*** Información de automovimiento como método de aprendizaje de representaciones
...
acelerómetros, etc) y se denomina \textit{información odométrica}. TODO: esto se podria borrar->La
información odométrica es ampliamente utilizada en robótica para
estimar la posición relativa de un agente respecto a una posición
inicial.

TODO: inspiracion biologica de como aprender buenas representacione viendo movimiento
TODO: como trasladar eso a nuestro dominio. Por que se eligen redes siamesas.
