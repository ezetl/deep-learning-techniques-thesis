...
\cite{sun397} (130 mil imágenes distribuidas en 397 clases).
...
evaluar experimentalmente las metodologías propuestas en Ref. \cite{LSM2015}.
...
transformaciones entre dos imágenes de entrada. En Ref. \cite{LSM2015}
...
Dado que el objetivo de este experimento es obtener resultados
preliminares que validen la técnica de \textit{automovimiento}, no se
realizarán pruebas utilizando la técnica SFA definida en la Sección
[[sec:sfa]]. Más aún, no es sencillo establecer una característica de
\textit{temporalidad} que permita diferenciar caracteres manuscritos.
