...
La Sección termina con una introducción a redes neuronales
...
haciendo foco en las dos utilizadas en los experimentos del Capítulo
\ref{sec:agrawal}: \textit{Slow Feature Analysis} y
...
Una manera de optimizar los parámetros \(W\) es estableciendo una
función de costo que minimice \(D(G_W(x_1), G_W(x_2))\) para pares
cercanos semánticamente y a su vez maximice esa distancia para pares
lejanos.
...
inicial. No solamente eso, sino que agentes biológicos también
utilizan su sistema perceptual para ubicarse en el ambiente y poder
lograr sus objetivos. Cabe preguntarse si toda la información que el
agente obtiene mediante su movimiento por el ambiente
...
Nos enfocaremos en la tarea de percepción visual. En este caso, la
...
Agrawal et al. \cite{LSM2015} proponen que relacionando los estímulos
...
cámara. Bajo este enfoque, el problema de relacionar los estímulos
visuales con el automovimiento puede ser establecido como el problema
de predecir las transformaciones de la cámara en pares de imágenes a
lo largo del movimiento de la misma.


De esta manera el problema puede ser implementado con una arquitectura
de redes convolucionales siamesas cuya tarea sea la de predecir las
transformaciones entre dos imágenes de entrada. \cite{LSM2015} prueba
para algunos casos que las representaciones obtenidas al entrenar la
red siamesa de manera no supervisada mediante la información de
automovimiento son competitivas con las del estado del arte que
utilizan métodos supervisados de clasificación cuando son entrenadas
con pocas imágenes por categoría.
...
\texti{Slow Feature Analysis}. Se concluye eventualmente que la
técnica de automovimiento es superior a \textit{Slow Feature
Analysis}.
...
se mueve a través de su entorno, usamos representaciones de
imágenes \(x_{t_1}\) y \(x_{t_2}\) tomadas en tiempos \(t_1\) y
\(t_2\) respectivamente y computamos su similitud mediante alguna
...
lo que se discretizan las traslaciones en \(X\) y \(Z\) y las
rotaciones en \(Y\) (el eje perpendicular a la cámara) por rangos de
