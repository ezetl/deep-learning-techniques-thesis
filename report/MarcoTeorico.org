...
de los cientos de miles hasta millones de imágenes. Generalmente se
...
puede anotar una gran cantidad de datos es la propuesta por
\cite{LSM2015}. En la misma proponen utilizar información odométrica
disponible en cámaras (giróscopos, acelerómetros, etc) para
pre-entrenar redes neuronales profundas, y luego entrenarlas
utilizando el conjunto de datos anotados que se desee, obteniendo la
ventaja de no necesitar un conjunto tan grande para lograr buena
precisión.
...
lo que vamos a hacer es calcular su gradiente \(\nabla L\) respecto a
cada parametro y luego modificar cada uno ligeramente con el objetivo
de alcanzar un minimo en la funcion:
...
ecuacion se puede observar que se modifican los parametros con
respecto a la direccion opuesta al gradiente, dado que el mismo indica
la direccion de crecimiento de una funcion pero nosotros queremos
encontrar un minimo.

Existen variantes mas sofisticadas del descenso de gradiente
(Momentum, Nesterov Momentum, Adagrad, Rmsprop entre otros). Lo mas
comun es utilizar una tecnica llamada \textit{descenso de gradiente
estocastico}, que se basa en calcular el gradiente de un subconjunto
...
es saturante y converge hasta 6 veces mas rapido que las funciones
...
negativos. Esta funcion de activacion es la que se conoce como
\textit{Leaky ReLU} \cite{zhang2014improving}:
...
Maxout \cite{Maxout} es una generalización de las funciones \textit{ReLU}, y obtiene lo
...
La técnica de \textout{dropout} \cite{dropout} consiste en mantener
...
Donde \(f\) es una función de activación. Si ahora agregamos \textit{dropout}:
...
**** TODO (AGREGAR IMAGEN) Arquitecturas 
...
* Redes neuronales siamesas                      :noexport:
...
Analizar como hacer redes siamesas en distintos frameworks. Hacer
pruebas de concepto con MNIST. Elegir un framework.

EL objetivo es entender como hacer redes siamesas en los distintos
frameworks (entrada de la red, como es la ultima capa, como
...
**** TODO Modificar codigo para crear las transformaciones aleatorias de rotacion y traslacion (OpenCV)
**** TODO Modificar codigo para guardar lmdb
*** [0%] Entrenamiento
**** TODO Preparar Arquitectura
BCCN: C96-P-C256-P
TCCN: F1000-D-Op
...
**** TODO Iteraciones, learning rate, step
