...
x2 = x1 - mx
y2 = y1 - my
axes[0].set_xlabel('a) Original', fontsize=15)
axes[0].scatter(x1, y1, s=20, c='royalblue', marker="s")

axes[1].set_xlabel('b) Substraccion de media', fontsize=15)
axes[1].scatter(x2, y2, s=20, c='seagreen', marker="s")

x3 = x2 / 2.0
y3 = y2 / 2.0

axes[2].set_xlabel('c) Normalizacion', fontsize=15)
...
fonts = 15
ax1.set_xlabel('a) Original', fontsize=fonts)
ax2.set_xlabel('b) Alineamiento', fontsize=fonts)
ax3.set_xlabel('c) Color-Brillo', fontsize=fonts)
ax4.set_xlabel('d) Traslaciones', fontsize=fonts)
...
#+attr_latex: width=0.8\textwidth,placement=[p]
#+label: fig:augm
#+caption: Diferentes formas de aumentar datos. Si trabajamos con rostros es muy común alinearlos, por ejemplo, sobre el eje que conforman los ojos (a). Traslaciones, recortes en la imagen y cambios en el brillo, contraste y color son otras técnicas muy usadas.
...
def explore_fitting_boundaries(model, n_neighbors, datasets, width):
    ...
    for n in n_neighbors:
        ...
        for d in datasets.keys():
            ...
            j += 1
        if i==0:
            panel.set_xlabel(r'a) $\mathit{Underfitting}$', fontsize=40)
        elif i==1:
            panel.set_xlabel(r'b) $\mathit{Aceptable}$', fontsize=40)
        elif i==2:
            panel.set_xlabel(r'c) $\mathit{Overfitting}$', fontsize=40)
        i += 1
...
#+attr_latex: width=\textwidth, height=2, placement=[p]
#+label: fig:overfit
...
#+attr_latex: width=\textwidth, height=2, placement=[p]
#+label: fig:nnet
#+caption: Diagrama de una red neuronal con entrada de dimensión \(D=3\), una capa oculta completamente conectada y dos neuronas de salida.
[[file:images/neural-net.pdf]]
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.3\textwidth}
        \begin{bmatrix}
         ...
         0  &  1 & 0 \\
        \end{bmatrix}
        \caption{Matriz convolucional \(K\)}
        \label{fig:K}
    \end{subfigure}
    \begin{subfigure}[b]{0.3\textwidth}
        ...
        \label{fig:conv-edge}
    \end{subfigure}
    \caption{Es muy común convolucionar imágenes con ciertos \texit{kernels} para detectar bordes en las mismas.}\label{fig:conv}
\end{figure}
#+END_LaTeX
