#+TITLE: Reconocimiento visual empleando técnicas de \textit{deep learning}
...
\ref{fig-one} de manera simplificada, nuestro clasificador lineal
...
#+label: fig:one
#+caption: Clasificador lineal. Cada punto representa una muestra en un espacio de dimension \(\boldsymbol{D}=2\) con \(\boldsymbol{K}=2\) categorias. La tarea del clasificador es establecer un hiperplano entre las dos clases de datos, definido por la ecuacion \(f(x_i, W, b)= W x_i + b\). A modo de ejemplo estan graficados dos clasificadores lineales mas con el \textit{vector de sesgo} ligeramente modificado. Se puede observar que \bias{} no afecta al clasificador sino que simplemente lo traslada a lo largo de las dimensiones.
...
**** TODO esquema del proceso de entrenamiento: medir error-> actualizar->medir error, etc
...
***** TODO figura de descenso de gradiente, con derivadas y direccion de actualizacion
...
**** TODO dibujo de las pendientes de las 3 funciones de activacion
...
***** TODO dibujo de datos con la media corrida y la media centrada
**** Normalización
...
***** TODO dibujo con normalizacion de datos
...
***** TODO dibujo con data augmentation, face alignment
