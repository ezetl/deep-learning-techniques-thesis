...
en ingles \textit{Base CNN}. Las \textit{features} extraídas de estas
...
\begin{itemize}
\item \(N\)C\(k\) para una capa convolucional con \(k\) filtros cuadrados de tamaño \(N*N\).
