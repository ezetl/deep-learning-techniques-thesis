...
*** TODO --> (RESUMIR) Clasificadores lineales

Dentro del paradigma del aprendizaje supervisado, vamos a dar una
pequeña introduccion a los clasificadores lineales orientada a
imagenes, junto con varias tecnicas de entrenamiento que ya veremos
tambien son ampliamente usadas en el entrenamiento de redes
neuronales.
...
imágenes \(\boldsymbol{x_i} \in \boldsymbol{R^{D}}\), donde
\(\boldsymbol{i = 1\cdots N}\), \(\boldsymbol{D}\) es la dimensión de
cada imagen y \(\boldsymbol{y_i = 1\cdots K}\) es la etiqueta
asociada. Es decir, tenemos \(\boldsymbol{N}\) imágenes y
...
** TODO --> (RESUMIR. Tal vez se pueda borrar esta seccion) Inspiración biológica de las neuronas
...
** TODO --> (TAL VEZ RESUMIR. o describir solamente ReLU en profundidad) Funciones de activación comunes
*** Sigmoide
...
*** TODO Transferencia de aprendizaje
